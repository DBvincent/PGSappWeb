...
var time=new Date().getMinutes();
	...
	backAjax("../../v1/dictionary/customerCategories",transfersID,headers);
...
function getData(){
	...
	DATA.categoryCode=$("#genre").attr("data-code")?$("#genre").attr("data-code"):"";
	DATA.subCategoryCode=$("#occupation").attr("data-code")?$("#occupation").attr("data-code"):"";
	DATA.subCategorySubCode=$("#level").attr("data-code")?$("#level").attr("data-code"):"";
	DATA.proId=getID;
  	DATA.type=getType;
  	console.log(DATA);
}
...
function handlePopup(ele) {
    ...
    $(ele).stop(true, true).animate({ height:604}, 500,"swing");
    $("#close").click(function() {
        $("#layer").hide();
        $(ele).animate({ height:-308}, 800,"swing");
    });
        ...
        $.ajax({
        	...
        	success:function(data){
        		...
        		if(data.succeed){
                    ...
                    totalVerify.closeButton();
                    nativeCall(this,Bridge,"hideProgressHUD","");
                    nativeCall(this, Bridge, "showToast",data.message);
                }
        	},
